...
const Rendertable = () => {
    ...
    dataGlasses.forEach((value) => {
        // console.log(value)
        html += `<div class="col-4 vglasses__items">
        <img src="${value.src}"  onclick="addColor('${value.id}')" style="width: 150px;"/>
        </div>`;
    });
...
const addColor = (idClick) => {
    dataGlasses.forEach((value) => {
        if (idClick === value.id) {
            console.log(idClick);
            document.querySelector(
                "#avatar"
            ).innerHTML = `<img src="${value.virtualImg}">`;
            const ele = document.querySelector("#glassesInfo");
            ele.innerHTML = `<h1>${value.name} - ${value.brand}${value.color}</h1>
